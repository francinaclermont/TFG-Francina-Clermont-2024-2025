...
experiment_label <- '3_DownUpSampling_1_1'
file_output <- 'exp_3_DownUpSampling_1_1'
...
# Passos de preprocessament, només fem downsample i upsample
...
eina_de_treball <- mostres_bootsrap %>%
  mutate(receta = map(data_training, 
                      ~ recipes::recipe(CMVD_rev ~ ., data = ..1) %>%
                        ...
                        themis::step_downsample(CMVD_rev, under_ratio = 1, skip = T) %>%
