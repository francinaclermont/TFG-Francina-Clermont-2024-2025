...
experiment_label <- '10_SMOTE_n5'
file_output <- 'exp_10_SMOTE_n5'
...
# Passos de preprocessament, smote amb 5 veïns
...
eina_de_treball <- mostres_bootsrap %>%
  mutate(receta = map(data_training, 
                      ~ recipes::recipe(CMVD_rev ~ ., data = ..1) %>%
                        ...
                        themis::step_smote(CMVD_rev, neighbors = 5, over_ratio = 1, skip = T)
  ))
