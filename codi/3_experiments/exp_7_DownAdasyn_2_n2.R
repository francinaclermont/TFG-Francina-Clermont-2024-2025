...
experiment_label <- '7_DownAdasyn_2_n2'
file_output <- 'exp_7_DownAdasyn_2_n2'
...
# Passos de preprocessament, apliquem downsample (majoritària duplicada) i adasyn (2 veïns)
...
eina_de_treball <- mostres_bootsrap %>%
  mutate(receta = map(data_training, 
                      ~ recipes::recipe(CMVD_rev ~ ., data = ..1) %>%
                        ...
                        themis::step_adasyn(CMVD_rev, neighbors = 2, over_ratio = 1, skip = T)
