...
# llista de tots els arxius en el directori indicat
archivos_paths <- list.files(path = 'programacio/outputs/Experiments/') %>%  as.list()

#creem una taula amb tots els coeficients extrets dels experiments
...
archivos_paths %>% 
  ...
  # `format()` evita notació científica y limita a 2 decimales
...
archivos_paths %>% 
  ...
  arrange(order, experimento) %>%  # Ordenar per 'order', i 'experimento'
  mutate(experimento = factor(experimento, levels = unique(experimento))) %>% # Convertir a factor con niveles en el orden actual
  select(-order, -statistic) %>%
